...
!!! note "图片占位"
    此处可放置 Specialization Example，对应 `person ISA customer / employee` 或 `account ISA saving-account / checking-account`。
...
!!! note "图片占位"
    此处可放置 Specialization / Generalization 的约束图，分别展示 disjoint、overlapping、total 和 partial。
...
!!! note "图片占位"
    此处可放置 Aggregation 图，将 `works-on` 作为复合实体集参与 `manages` 联系。
...
!!! note "图片占位"
    此处可放置 ER Diagram Notation 和 UML Class Diagram 的对照图。需要注意 UML 中 cardinality 标注的位置和 ER 图是反过来的。
